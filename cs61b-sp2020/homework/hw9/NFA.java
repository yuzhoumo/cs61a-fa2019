...
public class NFA {
    ...
    private class State {
        ...
        public Set<State> successors(char c) {
            if (c == EPSILON && _edges.containsKey(c)) {
                HashSet<State> results = new HashSet<>();
                HashSet<State> marked = new HashSet<>();
                Stack<State> fringe = new Stack<>();
                fringe.push(this);

                while (!fringe.isEmpty()) {
                    State successor = fringe.pop();

                    if (!marked.contains(successor)) {
                        marked.add(successor);
                        results.add(successor);

                        Set<State> nodes = successor._edges.get(c);
                        nodes = nodes != null ? nodes : new HashSet<State>();
    ...
    public boolean matches(String s) {
        ...
        for (char c : s.toCharArray()) {
            HashSet<State> S_new = new HashSet<>();

            for (State q : S) {
                Set<State> q_new = q.successors(c);
                S_new.addAll(q_new);
            }
            ...
            for (State q_new : S) {
                Set<State> q_e = q_new.successors(EPSILON);
                S_new.addAll(q_e);
            }
