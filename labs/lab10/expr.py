...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        try:
            return self.operator.eval(env).apply([e.eval(env) for e in self.operands])
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        env, new_vals = self.parent.copy(), zip(self.parameters, arguments)
        env.update(dict(new_vals))
