...
class VendingMachine:
    ...
    def vend(self):
        if self.amount == 0:
            ...
            self.money, change = 0, self.money
            return 'Machine is out of stock. Here is your ${0}.'.format(change)

        if self.money < self.price:
            return 'You must deposit ${0} more.'.format(self.price - self.money)

        self.amount, self.money = self.amount - 1, self.money - self.price
    ...
    def deposit(self, money_in):
        if self.amount == 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(money_in)
        self.money += money_in
        return 'Current balance: ${0}'.format(self.money)

    def restock(self, amount_in):
        self.amount += amount_in
        return 'Current {0} stock: {1}'.format(self.item, self.amount)
...
def store_digits(n):
    ...
    """
    n = [int(d) for d in str(n)] if type(n) != list else n
    if len(n) == 1:
        return Link(n[0])
