...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        ...
        if (node1 == null && node2 == null)
            throw new NullPointerException("Both nodes are null.");
        if (node1 == null)
            return index2;
        if (node2 == null)
            return index1;
    ...
    private void bubbleDown(int index) {
        while (getLeftOf(index) <= size()) {
            ...
            Node minChild = getNode(nextIndex);
            Node current = getNode(index);

            if (current == null || minChild == null)
                break;
    ...
    public void insert(T item, double priority) {
        int last = size() + 1;
        setNode(last, new Node(item, priority));
        bubbleUp(last);
    }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size(); i++) {
            Node curr = getNode(i);
            if (curr != null && item.equals(curr.item())) {
                double oldPriority = curr.priority();
