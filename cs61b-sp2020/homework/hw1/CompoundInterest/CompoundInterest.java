...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double future = futureValue(presentValue, rate, targetYear);
        return future * Math.pow(1 - inflationRate / 100, numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for (int i = 1; i <= numYears(targetYear); i++) {
            tot += perYear * Math.pow(rate, i);
        }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double tot = totalSavings(perYear, targetYear, rate);
        return tot * Math.pow(1 - inflationRate / 100, numYears(targetYear));
    }
