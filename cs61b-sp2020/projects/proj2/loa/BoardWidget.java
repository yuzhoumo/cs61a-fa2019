...
class BoardWidget extends Pad {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (Square sq : _legalSquares) {
            drawMarker(g, sq);
        }
    }

    /** Draw a marker using G to indicate a legal move S. */
    private void drawMarker(Graphics2D g, Square s) {
        g.setColor(Color.green);
        g.fillOval(cx(s) + PIECE_OFFSET + 5, cy(s) + PIECE_OFFSET + 5,
                PIECE_SIZE - 10, PIECE_SIZE - 10);
        g.setColor(PIECE_BOUNDARY_COLOR);
        g.setStroke(PIECE_BOUNDARY_STROKE);
        g.drawOval(cx(s) + PIECE_OFFSET + 5, cy(s) + PIECE_OFFSET + 5,
                PIECE_SIZE - 10, PIECE_SIZE - 10);
    }
    ...
    private void mousePressed(Square s) {
        if (!_board.gameOver() && _board.turn() == _board.get(s)) {
            _startSquare = s;
            _legalSquares = new ArrayList<>();
            for (Move m : _board.legalMoves(s)) {
                _legalSquares.add(m.getTo());
            }
            repaint();
        }
    }
    ...
    private void mouseReleased(Square s) {
        if (!_board.gameOver() && _startSquare != null) {
            if (_legalSquares.contains(s)) {
                Move move = Move.mv(_startSquare, s);
    ...
    /** Used to store square on MousePressed. */
    private Square _startSquare = null;

    /** Used to store squares to paint legal move markers. */
    private List<Square> _legalSquares = new ArrayList<>();
}
