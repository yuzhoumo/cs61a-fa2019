...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        Sq connectableSq = null;
        int seqNum = start.sequenceNum();
        int count = 0;

        for (Place p : start.successors()) {
            Sq currentSq = model.get(p);

            if (seqNum != 0 && currentSq.sequenceNum() == seqNum + 1) {
                return currentSq;
            }

            if (start.connectable(currentSq)) {
                connectableSq = currentSq;
                count++;
            }
        }

        return count > 1 ? null : connectableSq;
    }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        for (Place p : end.predecessors()) {
            if (model.get(p).connectable(end)) {
                connectableSq = model.get(p);
                count++;
            }
        }
        return count > 1 ? null : connectableSq;
