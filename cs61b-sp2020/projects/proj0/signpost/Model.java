...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int solNum = _solution[i][j];
                _solnNumToPlace[solNum] = pl(i, j);
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int solNum = _solution[i][j];
                boolean fixed = solNum == 1 || solNum == _width * _height;
                int groupNum = fixed ? 0 : -1;
                int direction = arrowDirection(i, j);

                Sq currentSq = new Sq(i, j, fixed ? solNum : 0, fixed,
                        direction, groupNum);

                currentSq._predecessors = new PlaceList();
                for (Place p : _allSuccessors[i][j][0]) {
                    if (p.dirOf(pl(i, j)) != 0) {
                        currentSq._predecessors.add(p);
                    }
                }

                currentSq._successors = _allSuccessors[i][j][direction];
                _board[i][j] = currentSq;
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                Sq currentSq = new Sq(model.get(i, j));
                _board[i][j] = currentSq;
            }
        }
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                ...
                Sq succ = model.get(square.pl).successor();
                Sq pred = model.get(square.pl).predecessor();
                Sq head = model.get(square.pl).head();

                square._successor = succ != null ? get(succ.pl) : null;
                square._predecessor = pred != null ? get(pred.pl) : null;
                square._head = get(head.pl);
    ...
    boolean autoconnect() {
        ...
        for (Sq square : _allSquares) {
            ...
            boolean conditions = square.sequenceNum() != 0
                    && square.successors() != null
                    && square.successor() == null;

            if (conditions) {
                for (Place p : square.successors()) {
                    if (square.sequenceNum() == get(p).sequenceNum() - 1) {
                        square.connect(get(p));
                        changed = true;
                    }
                }
            }
    ...
    void solve() {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                get(i, j)._sequenceNum = solution()[i][j];
                get(i, j)._head = get(solnNumToPlace(1));

                if (get(i, j).sequenceNum() != 1) {
                    int predecessorNum = get(i, j).sequenceNum() - 1;
                    get(i, j)._predecessor =
                            get(solnNumToPlace(predecessorNum));
                }

                if (get(i, j).sequenceNum() != last) {
                    int successorNum = get(i, j).sequenceNum() + 1;
                    get(i, j)._successor = get(solnNumToPlace(successorNum));
                }
            }
        }
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (seq0 != width() * height()) {
            Place dest = solnNumToPlace(seq0 + 1);
            result = Place.dirOf(x, y, dest.x, dest.y);
        }
        return result;
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean conditions = pl.dirOf(s1.pl) == direction()
                    && s1.predecessor() == null
                    && successor() == null
                    && s1.sequenceNum() != 1
                    && sequenceNum() != width() * height();
        ...
        boolean connect(Sq s1) {
            ...
            int sgroup = s1.group();
            boolean sUnnumbered = s1.sequenceNum() == 0;

            _successor = s1;
            s1._predecessor = this;

            if (sequenceNum() != 0) {
                Sq pointer = successor();
                int num = sequenceNum() + 1;
                while (pointer != null) {
                    pointer._sequenceNum = num;
            ...
            Sq thisHead = head();
            while (pointer != null) {
                pointer._head = thisHead;
                pointer = pointer.successor();
            }

            if (s1.sequenceNum() != 0 && sUnnumbered) {
                releaseGroup(sgroup);
            }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                boolean thisOneElement = predecessor() == null;
                boolean nextOneElement = next.successor() == null;

                if (thisOneElement && nextOneElement) {
                    releaseGroup(_head._group);
                    next._group = _group = -1;
                } else if (thisOneElement) {
                    next._group = _group;
                    _group = -1;
                } else if (nextOneElement) {
                    next._group = -1;
                } else {
                ...
                Sq pointer = this;
                boolean noFixedSequenceNums = true;
                noFixedSequenceNums = noFixedNums(this, -1);
                if (noFixedSequenceNums) {
                    while (pointer != null) {
                ...
                noFixedSequenceNums = noFixedNums(next, 1);
            ...
            setAllSuccessorHeads(next, next);
        }

        /**
         * Sets the head of START and all of START's successors to HEAD.
         */
        void setAllSuccessorHeads(Sq start, Sq head) {
            Sq pointer = start;
            while (pointer != null) {
                pointer._head = head;
                pointer = pointer.successor();
        ...
        /**
         * Checks if any of SQUARE's successors or predecessors have a fixed
         * number. DIR = -1 checks predecessors while DIR = 1 checks successors.
         *
         * @return {boolean} False if any of SQUARE's successors/predecessors
         * have fixed sequence numbers, otherwise true
         */
        boolean noFixedNums(Sq square, int dir) {
            ...
            if (dir == 1) {
                while (pointer != null) {
                    if (pointer.hasFixedNum()) {
                        noFixedSequenceNums = false;
                        break;
                    }
                    pointer = pointer.successor();
                }
